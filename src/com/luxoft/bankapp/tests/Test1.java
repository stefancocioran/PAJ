...
public class Test1 {
	...
	public void testCheckingAccount() throws OverdraftLimitExceededException {
		CheckingAccount checkingAccount = new CheckingAccount(2, 1000.0,
		                                                      100.0);
	...
	public void testClient() {
		Client client = new Client("Smith John", Gender.MALE, "Bucharest");
	...
	public void testBank() throws ClientExistsException {
		Bank bank = new Bank();
		Client client1 = new Client("Smith John", Gender.MALE, "Oradea");
		client1.addAccount(new SavingAccount(1, 1000.0));
		client1.addAccount(new CheckingAccount(2, 1000.0, 100.0));

		Client client2 = new Client("Smith Michelle", Gender.FEMALE, "Brasov");
		...
		var it = bank.getClients().iterator();
		var first = it.next();
		var second = it.next();

		assertEquals(2, bank.getClients().size());
		assertEquals("Mr. Smith John", second.getClientGreeting());
		assertEquals("Mr. Smith John", second.toString());
		assertEquals("Ms. Smith Michelle", first.getClientGreeting());
		assertEquals("Ms. Smith Michelle", first.toString());
	}
